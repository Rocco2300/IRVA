...
namespace L3_VR_SteamVR_Advanced.Scripts.GravityGun
{
    public class GravityGunController : MonoBehaviour
    {
        ...
        public void OnTouchTrigger(SteamVR_Action_Boolean fromTouch, SteamVR_Input_Sources fromSource, bool newState)
        {
            if (newState)
            {
                SnapObject();
        ...
        public void OnGrabPinch(SteamVR_Action_Boolean fromPinch, SteamVR_Input_Sources fromSource, bool newState)
        {
            if (newState)
            {
                ThrowObject(); 
        ...
        private void OnEnable()
        {
            ...
            SteamVR_Actions._default.GrabPinch.onChange += OnGrabPinch;
            SteamVR_Actions._default.TouchTrigger.onChange += OnTouchTrigger;
        }
        ...
        private void Update()
        {
            ...
            if (_isGunGrabbed && hitObject != null)
            {
                var joystick = SteamVR_Actions._default.Joystick.GetAxis(SteamVR_Input_Sources.RightHand);
                Debug.Log(joystick);
                var eulerAngles = new Vector3(joystick.x, joystick.y, 0f);
                _selectedObject?.transform.Rotate(eulerAngles * Time.deltaTime * 10f, Space.Self);      
                //snapTransform.transform.Rotate(eulerAngles * Time.deltaTime * 10f);
                //hitObject.transform.Rotate(eulerAngles * Time.deltaTime * 90f, Space.Self);
            }
        }
